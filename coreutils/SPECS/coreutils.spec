...
#ls -l should not leak memory(#751974)
Patch926: coreutils-8.4-ls-leak.patch
#display target device for symlink parameters (#766461)
Patch927: coreutils-8.4-df-symlinkinfo.patch
#recognize few new filesystems in stat, FhGFS and GPFS as remote in tail(#769874)
Patch928: coreutils-8.4-tailstatnewfs.patch
#add support for clearing special bits on directories via 5+ digit octal mode
Patch929: coreutils-8.4-exactoctaldigitsmode.patch
#Revert to polling in tail -f for unknown filesystems, update known filesystems from 8.21 (#827199)
Patch930: coreutils-8.4-tail-f-polling-unknownfs.patch
#fix parsing of field regression in sort command(#956143)
Patch931: coreutils-8.4-sort-end-of-fields.patch
# fix status codes (#889531) and propagation of child core dump info (#747592)
Patch932: coreutils-8.4-su-statuscodes.patch
#add status=none to dd to suppress all status info (#965654)
Patch933: coreutils-8.4-ddstatusnone.patch
#remove redundant setpwent() and setgrent() from stat
#to improve NIS performance (#911206)
Patch934: coreutils-8.4-NISperformance.patch
#date: deal correctly with invalid input with special characters (#960160)
Patch935: coreutils-8.4-date-nosignextend.patch
#dd: provide support for conv=sparse (#908980)
Patch936: coreutils-8.4-ddsparse.patch
#su/runuser: clarify which envvars are preserved/initialized
#in -p/-m and -l help/man documentation (#967623)
Patch937: coreutils-8.4-su-clarifypreservedenvvar.patch
#du: detect bind mounts (#836557)
Patch938: coreutils-8.4-du-bindmounts.patch
#df: fix alignment of columns (#842040)
Patch939: coreutils-8.4-df-columns.patch
#id: print correct groups (#816708)
Patch940: coreutils-8.4-id-wronggroup.patch
#mv: replace empty directories in cross file system move (#980061)
Patch941: coreutils-8.4-mv-replaceemptydirs.patch
#df: fix output of --direct after applying Patch939
Patch942: coreutils-8.4-df-directfix.patch
#pr - fix the crash and padding with -n option (#997537)
Patch943: coreutils-8.4-pr-n-padcrash.patch
#su: fix wrong return codes when NOT killed by signal (#996190)
Patch944: coreutils-8.4-su-statuscodes-fix.patch
#df: dereference long names (#842040)
Patch945: coreutils-8.4-df-name-deref.patch
#tail: don't use inotify if tailing symlink(s) (#10006221)
Patch946: coreutils-8.4-tail-symlink-polling.patch
...
#other patches
%patch926 -p1 -b .lsleak
%patch927 -p1 -b .symlinkdevice
%patch928 -p1 -b .newfs
%patch929 -p1 -b .specialbits
%patch930 -p1 -b .rfspolling
%patch931 -p1 -b .endoffields
%patch932 -p1 -b .statuscodes
%patch933 -p1 -b .statusnone
%patch934 -p1 -b .NIS
%patch935 -p1 -b .nosign
%patch936 -p1 -b .sparse
%patch937 -p1 -b .envvars
%patch938 -p1 -b .bindmounts
%patch939 -p1 -b .columns
%patch940 -p1 -b .wronggroup
%patch941 -p1 -b .emptydir
%patch942 -p1 -b .directfix
%patch943 -p1 -b .padding
%patch944 -p1 -b .statuscodes-fix
%patch945 -p1 -b .deref
%patch946 -p1 -b .polling

chmod a+x tests/misc/sort-mb-tests tests/df/direct tests/ls/slink-acl tests/dd/sparse tests/misc/id-setgid
...
%changelog
* Thu Oct 17 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-31
...
* Fri Oct 04 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-29
- CVE-2013-0221 CVE-2013-0223 CVE-2013-0222 - fix various
  segmentation faults in sort, uniq and join(#1015019)

* Tue Sep 17 2013 Ondrej Oprala <ooprala@redhat.com> - 8.4-28
- su now returns correct retvals for all cases

* Tue Sep 17 2013 Ondrej Oprala <ooprala@redhat.com> - 8.4-27
...
* Fri Aug 16 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-25
...
* Wed Aug 14 2013 Ondrej Oprala <ooprala@redhat.com> - 8.4-24
- fix su return codes when NOT killed by a signal (#996190)

* Mon Aug 05 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-23
- fix several newly introduced defects found by Coverity
  check

* Thu Aug 01 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-22
...
* Wed May 15 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-21
- fix parsing of field regression in sort command
  (introduced between RHEL5 and RHEL6 upstream) (#956143)

* Fri Mar 22 2013 Ondrej Vasik <ovasik@redhat.com> - 8.4-20
- revert to polling for unknown filesystems, update
  known fs for tail and stat based on coreutils-8.21 (#827199)

* Tue Apr 17 2012 Ondrej Vasik <ovasik@redhat.com> - 8.4-19
- fix the tcsh colorls.csh behaviour in non-interactive
  mode (#804604)

* Mon Mar 05 2012 Ondrej Vasik <ovasik@redhat.com> - 8.4-18
- add support for cleaning special bits on directories
  via 5+ digits octals (#691466)

* Thu Feb 16 2012 Ondrej Vasik <ovasik@redhat.com> - 8.4-17
- fix memory leak in ls -l (#751974)
- document difference between ls -lZ and ls -Zl (#771843)
- fix various i18n patch related issues in pr (#772172)
- df should display target device information when
  a symbolic link is specified as a parameter(#766461)
- su no longer hangs when a program set itself to ignore
  SIG_CHLD signal, and fork a child to exec su(#754029)
- stat recognizes few new filesystems, tail -f recognizes
  FhGFS and GPFS as remote (disables inotify) (#769874)
